...
gulp.task('pack-js',
    gulp.parallel(
        bundlingConfigs.map(function(name) { return 'scripts:' + name; })
    )
);
...
bundlingConfigs.forEach(function(bundleName) {
    gulp.task('styles:' + bundleName, function() {
        return gulp.src(bundles[bundleName].css.src)
...
gulp.task('pack-css',
    gulp.parallel(
        bundlingConfigs.map(function(name) { return 'styles:' + name; })
    )
);
...
// basic build task.
gulp.task('build',
    gulp.parallel('pack-js', 'pack-css', 'leaflet-assets')
);
...
gulp.task('serve-and-sync',
    gulp.parallel('runserver', 'browser-sync')
);

// gulp watch task
gulp.task('watch',
    // start browserSync, and run the rest of our tasks once
    gulp.parallel('serve-and-sync', 'pack-css', 'pack-js', 'leaflet-assets',
